...
def collect_epubs(input_root, output_root):
    book_name = ""
    for root,__,files in os.walk(input_root, topdown=False):
        for name in files:
            if ".epub" in name:
                dst_name = " - ".join(root.split("\\")[-2:])

                # book_name = name.split(".")[0]
                book_file = os.path.join(root,name)
                shutil.copyfile(
                    src = book_file,
                    dst = os.path.join(output_root,f'{dst_name}.epub')
                )

            ## metadata is generated by calibre

            # elif ".opf" in name:
            #     dst_name = " - ".join(root.split("\\")[-2:])
            #     book_metadata = os.path.join(root,name)
            #     shutil.copyfile(
            #         src = book_metadata,
            #         dst = os.path.join(output_root,f'{dst_name}.opf')
            # )


def conver_all_epubs(input_root,output_root):
